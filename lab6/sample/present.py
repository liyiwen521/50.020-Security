...
def genRoundKeys(key):
    ...
    for i in range(1, FULLROUND + 2):
        key_list.append(new_key >> 16)
        new_key = rol(new_key, 61, 80)
        new_key = format(new_key, "080b")
        new_key = format(sbox[int(new_key[0:4], 2)], "04b") + new_key[4:]
        counter = format(i, "080b")
        for j in range(60, 65):
            if new_key[j] == '1' and counter[j-65] == '1':
                new_key = new_key[:j] + '0' + new_key[j+1:]
            elif new_key[j] == '0' and counter[j-65] == '0':
                new_key = new_key[:j] + '0' + new_key[j+1:]
            else:
                new_key = new_key[:j] + '1' + new_key[j+1:]
...
def sBoxLayer(state):
    tmp = format(state, "016x")
    new_state = ''
    for i in tmp:
        new_state += str(hex(sbox[int('0x'+i, 16)])[2:])
...
def sBoxLayer_inv(state):
    ...
    new_state = ''
    for i in tmp:
        new_state += str(hex(sbox_inv[int('0x'+i, 16)])[2:])
    new_state = int(new_state, 16)
    return new_state
	
	
def pLayer(state):
    tmp = format(state, "064b")
    new_state = []
    for i in range(len(tmp)):
        new_state.append('')
    for i in range(len(tmp)):
        new_state[pmt[i]] = tmp[i]
    p = ''
    for i in range(len(tmp)):
        p += new_state[i]
    p = int(p, 2)
